...
set.seed(14)
kappa[1] <- nu + kappa0 + rnorm(1, 0, sqrt(sigma_w)) 
for(k in 2:t){
  kappa[k] <- nu + kappa[k-1] + rnorm(1, 0, sqrt(sigma_w))
}
...
### estimation (n-steps) #age-time-spatial parameters (no gamma, no filtering) ----
it = 20000 #iterations
...
for(i in 2:it){
  ...
  C <- matrix(kronecker(theta.chain[i-1, ], rep(1, ages)))
  ...
  for (j in 1:ages) {
    ...
    X <- cbind(rep(1, each = t), rep(kappa.chain[i,], n))
    #X <- cbind(1, rep(kappa.chain[i,], n), rep(theta, each = t))
    aux.reg <- chol2inv(chol(t(X) %*% X))
    mean.reg <- aux.reg %*% t(X) %*% matrix( (Y.aux - rep(theta.chain[i-1,], each = t)) )
  ...
  for(j in 1:n){
    ...
    aux = ( sigma0*t*n + sigma_e.chain[i] )
    mu1 = ( sigma0*sum(y.hat) + sigma_e.chain[i]*mu0 )/aux
...
plot(1:n, theta, ylim = c(min(theta.est), max(theta.est)))
