...
sblc <- function(y, ages, t, n, m0, C0, M, W, it, bn, thin){
  ...
  for(i in 2:it){
    ...
    incline <- (kappa.chain[i,1] - kappa.chain[i,t+1]) / t
    ...
    sigma_w.chain[i] <- invgamma::rinvgamma(1, t/2, rate = aux/2)
    ### informative priors - gamma(a, b), a = 0.01, b = 0.01
    
    ### NU
    aux <- sigma_w.chain[i]/t
...
sblc_3 <- function(y, ages, t, n, m0, C0, M, W, it, bn, thin){
  ...
  mt.chain <- matrix(NA, nrow = it, ncol = t)
  Ct.chain <- matrix(NA, nrow = it, ncol = t)
  ...
  for(i in 2:it){
    ...
    ### SIGMA_T
    #aux <- M - W
    aux.2 <- theta.chain[i-1, ] %*% (M - W) %*% matrix(theta.chain[i-1, ])
    sigma_t.chain[i] <- invgamma::rinvgamma(1, (n-1)/2, rate = aux.2/2)
  ...
  mt <- mt.chain[seq(bn, it, by = thin), ]
  Ct <- Ct.chain[seq(bn, it, by = thin), ]
...
sblc_3_2 <- function(y, ages, t, n, m0, C0, M, W, it, bn, thin){
  ...
  for(i in 2:it){
    ...
    theta.chain[i-1,] <- (theta.chain[i-1,]) / incline2
    
    #alpha.chain[i-1,] <- alpha.chain[i-1,] + (beta.chain[i-1,] * level) + (gamma.chain[i-1,] * level2)
    alpha.chain[i-1,] <- alpha.chain[i-1,] + (beta.chain[i-1,] * level) 
    beta.chain[i-1,] <- beta.chain[i-1,] * incline
    gamma.chain[i-1,] <- gamma.chain[i-1,] * incline2
    #gamma.chain[i-1, 1] <- 1 - sum(gamma.chain[i-1, -1])
    ...
    for (j in 1:ages) {
      ...
      X <- cbind(1, rep(kappa.chain[i, ], n), rep(theta.chain[i-1, ], each = t))
      aux.reg <- Matrix::chol2inv(Matrix::chol(t(X) %*% X))
      mean.reg <- aux.reg %*% t(X) %*% matrix(Y.aux)
      var.reg <- sigma_e.chain[i, j] * aux.reg
    ...
    ### THETA_q
    for(j in 1:(n-1)){
      mu0 = sum( (Wi[j, -j]/Mi[j,j])*theta.chain[i-1, -c(j,n)] )
    ...
    theta.chain[i, n] <- 0   ### theta base RJ, muda interpret pra RJ
...
sblc_missing <- function(y, ages, t, n, m0, C0, M, W, it, bn, thin){
  ...
  for(i in 2:it){
    ...
    for(j in 1:nrow(ind_missing)){
      loc <- c(ifelse(ind_missing[j,1]%%ages == 0, ages, ind_missing%%ages),
               ind_missing[j,2],
...
sblc_missing_2 <- function(y, ages, t, n, m0, C0, M, W, it, bn, thin){
  ...
  for(i in 2:it){
    ...
    level2 <- mean(theta.chain[i-1,])  ## ruim
    incline2 <- 1/sum(gamma.chain[i-1,]) ## ruim
    ...
    for(j in 1:nrow(ind_missing)){
      ...
      aux = alpha.chain[i, loc[1]] + (beta.chain[i, loc[1]] * kappa.chain[i, loc[2]]) + (gamma.chain[i, loc[1]] * theta.chain[i, loc[3]])
      input.chain[i,j] <- rnorm(1, mean = aux, sd = sqrt(sigma_e.chain[i, loc[1]]))
    }
    y_obs[ind_missing] <- input.chain[i,]
