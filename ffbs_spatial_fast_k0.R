...
bs_sp <- function(mt, Ct, at, Rt, m0, C0){
  ...
  B0 = C0 * 1/Rt[1]
  s0 = m0 + B0 %*% (kappa.bs[1] - at[1])
  S0 = C0 - B0 * Rt[1] + t(B0)
  
  kappa.bs0 <- rnorm(1, s0, sqrt(S0))
...
sigma_t = 0.5
set.seed(14)
theta <- MASS::mvrnorm(n = 1, mu = rep(0, n), Sigma = chol2inv(chol(Qi))*sigma_t) 
...
for(k in 1:t){
  y_obs[,k] <- A + (B * kappa[k+1]) + C + rnorm(n*ages, 0, sd = sqrt(0.05))
}
...
### estimation (n-steps) #spatial parameters (no lambda, theta det., no sp error) (after filtering) ----
it = 10000 #iterations
kappa.chain <- matrix(NA, nrow = it, ncol = t)
...
### estimation (n-steps) #age-time-spatial parameters (no lambda, theta det., no  sp error) (after filtering) ----
it = 10000 #iterations
kappa.chain <- matrix(NA, nrow = it, ncol = t)
...
### estimation (n-steps) #age-time-spatial parameters (no lambda, theta det.) (after filtering) ----
it = 10000 #iterations
kappa.chain <- matrix(NA, nrow = it, ncol = t)
...
for(i in 2:it){
  ...
  ### theta_q 
  for(j in 1:n){
    mu0 = lambda.chain[i-1]*sum( (Wi[j, -j]/Mi[j,j])*theta.chain[i-1, -j] )
  ...
  ### lambda M-H
  ### fazer normal
  lambda.prop <- runif(1, min = lambda.min, max = lambda.max)
  # # 
  aux.1 <- sigma_t.chain[i] * chol2inv(chol(Mi - lambda.prop*Wi))
  aux.2 <- sigma_t.chain[i] * chol2inv(chol(Mi - lambda.chain[i-1]*Wi))
  # # fixar chol2inv pra poder reutilizar sem ficar fazendo conta
  acc <- mvtnorm::dmvnorm(theta.chain[i,], sigma = aux.1, log = T) - mvtnorm::dmvnorm(theta.chain[i,], sigma = aux.2, log = T)
  if(runif(1) < exp(acc)){
    lambda.chain[i] <- lambda.prop
    prop = prop + 1
  }else{
    lambda.chain[i] <- lambda.chain[i-1]
  }
...
## 2m 10000 iterations
prop/it ### aceitacao ta muito alta nessa proposta... hmm...
...
plot.ts(theta.chain[bn:it, 1])
plot.ts(theta.chain[bn:it, 5])
plot.ts(theta.chain[bn:it, 10])
plot.ts(theta.chain[bn:it, 15])
...
#par(mfrow = c(1, 3))
plot.ts(nu.chain[seq(bn, it, by = thin)], ylim = c(-0.45, -0.7))
abline(h = nu, col = 2)
plot.ts(sigma_w.chain[seq(bn, it, by = thin)])
abline(h = sigma_w, col = 2)
plot.ts(sigma_e.chain[seq(bn, it, by = thin)])
...
### alpha beta e gamma sairam esquisitos dessa vez
### FF demorando horrores nas contas matriciais
### estimation (n-steps) #age-time-spatial parameters (no filtering) ----
it = 20000 #iterations
kappa.chain <- matrix(NA, nrow = it, ncol = t)
...
for(i in 2:it){
  ...
  #### tentar mudar isso aqui pra "segurar" os thetas novos, quando j > 1
  #### j = 2, usar theta[i, 1] ao inves do theta[i-1, 1]
...
### alpha beta e gamma sairam esquisitos dessa vez
### FF demorando horrores nas contas matriciais
### estimation (n-steps) #age-time-spatial parameters (theta det.) ----
it = 3000 #iterations
#it = 10000
kappa.chain <- matrix(NA, nrow = it, ncol = t)
...
plot.ts(kappa.chain[bn:it ,1])
abline(h = kappa[1], col = 2)
plot.ts(kappa.chain[bn:it ,7])
abline(h = kappa[7], col = 2)
plot.ts(kappa.chain[bn:it ,14])
abline(h = kappa[14], col = 2)
plot.ts(kappa.chain[bn:it ,20])
abline(h = kappa[20], col = 2)
# 
acf(kappa.chain[bn:it ,1])
acf(kappa.chain[bn:it ,4])
acf(kappa.chain[bn:it ,7])
